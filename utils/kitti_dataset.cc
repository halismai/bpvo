...
bool KittiDataset::init(const ConfigFile& cf)
{
  try
  {
    ...
    auto left_fmt = Format("%s/sequences/%02d/image_0/%s.png", root_dir.c_str(), sequence, "%06d");
    auto right_fmt = Format("%s/sequences/%02d/image_1/%s.png", root_dir.c_str(), sequence, "%06d");
