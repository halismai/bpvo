//
// NOTE code is mostly from the SPS library for Koichiro Yamaguchi with some
// re-factoring and interfacing with opencv by halismai
//
// The copyright notice is below
// SEE source on the web http://ttic.uchicago.edu/~dmcallester/SPS/
//
// Changes:
//  - Memory is now retained for use across difference calls. NOTE: the code is
//  no longer thread safe.We shave off 200ms be keeping the memory allocated
//
//  - removed unnceessary memset calls
//
...
SgmStereo::Config::Config()
  ...
  , censusRadius(2)
  , windowRadius(2)
  , smoothnessPenaltySmall(100)
  , smoothnessPenaltyLarge(1600)
...
private:
  ...
  void computeLeftCostImage(const unsigned char* leftGrayscaleImage, const unsigned char* rightGrayscaleImage);
	...
	int height_;
	int widthStep_;
  typename bpvo::AlignedVector<unsigned short>::type leftCostImage_;
  typename bpvo::AlignedVector<unsigned short>::type rightCostImage_;
	typename bpvo::AlignedVector<unsigned short>::type rowAggregatedCost_;
	typename bpvo::AlignedVector<unsigned char>::type pixelwiseCostRow_;
	typename bpvo::AlignedVector<unsigned char>::type halfPixelRightMin_;
	typename bpvo::AlignedVector<unsigned char>::type halfPixelRightMax_;
	typename bpvo::AlignedVector<short>::type sgmBuffer_;

  typename bpvo::AlignedVector<unsigned short>::type leftDisparityImage_;
  typename bpvo::AlignedVector<unsigned short>::type rightDisparityImage_;
...
struct SgmStereo::Impl
{
  ...
  void compute(const SgmStereo::Config& config, const cv::Mat& left,
               const cv::Mat& right, cv::Mat& D)
  {
    stereo.setDisparityTotal( config.numberOfDisparities);
    stereo.setDisparityFactor( config.disparityFactor );
    stereo.setDataCostParameters( config.sobelCapValue, config.censusRadius,
                                 config.censusWeightFactor, config.windowRadius);
    stereo.setSmoothnessCostParameters(config.smoothnessPenaltySmall,
                                       config.smoothnessPenaltyLarge);
    stereo.setConsistencyThreshold(config.consistencyThreshold);

    D.create(left.size(), CV_32FC1);
    stereo.compute(left, right, D.ptr<float>());
  }
...
void SgmStereo::compute(const cv::Mat& left, const cv::Mat& right, cv::Mat& D)
{
  if(_config.numberOfDisparities <= 0 || _config.numberOfDisparities % 16)
  {
    throw std::invalid_argument("numberOfDisparities must be a multiple of 16");
  }
...
void SGMStereo::compute(const cv::Mat& leftImage, const cv::Mat& rightImage,
                        float* disparityImage)
{
  ...
  leftDisparityImage_.resize( width_ * height_ );
	performSGM(leftCostImage_.data(), leftDisparityImage_.data());
  ...
  rightDisparityImage_.resize( width_ * height_ );
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			disparityImage[width_*y + x] = static_cast<float>(leftDisparityImage_[width_*y + x]/disparityFactor_);
		}
	}
...
{
  leftCostImage_.resize( width_ * height_ * disparityTotal_ );
  rightCostImage_.resize( width_ * height_ * disparityTotal_ );
	...
	sgmBuffer_.resize(totalBufferSize_);
}
...
void SGMStereo::computeLeftCostImage(const unsigned char* leftGrayscaleImage,
                                     const unsigned char* rightGrayscaleImage)
{
	...
	unsigned short* costImageRow = leftCostImage_.data();
...
void SGMStereo::computeCappedSobelImage(const unsigned char* image, const bool horizontalFlip,
							   ...
							   unsigned short* costImageRow)
{
	for (int rowIndex = 0; rowIndex <= aggregationWindowRadius_; ++rowIndex) {
		...
		unsigned short* rowAggregatedCostCurrent = rowAggregatedCost_.data() + rowAggregatedCostIndex*width_*disparityTotal_;
		...
		for (int x = 1; x < width_; ++x) {
			const unsigned char* addPixelwiseCost = pixelwiseCostRow_.data()
				+ std::min((x + aggregationWindowRadius_)*disparityTotal_, (width_ - 1)*disparityTotal_);
			const unsigned char* subPixelwiseCost = pixelwiseCostRow_.data()
							 ...
							 unsigned short* costImageRow)
{
	...
	for (int y = 1; y < height_; ++y) {
		...
		unsigned short* addRowAggregatedCost = rowAggregatedCost_.data() + width_*disparityTotal_*addRowAggregatedCostIndex;
		...
		if (addRowIndex < height_) {
			...
			const unsigned short* subRowAggregatedCost = rowAggregatedCost_.data() + width_*disparityTotal_*subRowAggregatedCostIndex;
...
void SGMStereo::calcPixelwiseSAD(const unsigned char* leftSobelRow, const unsigned char* rightSobelRow) {
	...
	for (int x = 16; x < disparityTotal_; ++x)
  {
		...
		for (int d = 0; d < x/16; d += 16)
    {
			...
			__m128i registerRightMinValue = _mm_loadu_si128(reinterpret_cast<const __m128i*>(
              halfPixelRightMin_.data() + width_ - 1 - x + d));
			__m128i registerRightMaxValue = _mm_loadu_si128(reinterpret_cast<const __m128i*>(
              halfPixelRightMax_.data() + width_ - 1 - x + d));
			...
			_mm_store_si128(reinterpret_cast<__m128i*>(pixelwiseCostRow_.data() + disparityTotal_*x + d), registerCost);
		}
...
void SGMStereo::computeRightCostImage()
{
	...
	for (int y = 0; y < height_; ++y)
  {
		unsigned short* leftCostRow = leftCostImage_.data() + widthStepCost*y;
		unsigned short* rightCostRow = rightCostImage_.data() + widthStepCost*y;
...
void SGMStereo::performSGM(unsigned short* costImage, unsigned short* disparityImage)
{
	...
	short* costSums = sgmBuffer_.data();
