...
static FORCE_INLINE __m128 makeWeights(const __m128& p)
{
...
static FORCE_INLINE
void do_partial_warp(const Matrix34& P, const float* xyzw, int N, int rows, int cols,
                     float* coeffs, int* uv, uint8_t* valid)
{
  ...
  for( ; i < n; i += S, xyzw += S*4, uv += S*2, coeffs += S*4, valid += 2) {
    ...
    auto p0p1_i = _mm_cvtps_epi32(p0p1);
  ...
  for( ; i < N; ++i) {
    printf("%d\n", i);
    // one point is left here, not worth it
    valid[i] = 0;
  }
                              ...
                              std::vector<uint8_t>& valid)
{
  ...
  do_partial_warp(P, xyzw[0].data(), N, rows, cols, interp_coeffs[0].data(), uv[0].data(), valid.data());
}
